...
const registrarAluno = async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ mensagem: "Aluno registrado com sucesso!", aluno: result.rows[0] });
    } catch (error) {
        console.error("Erro ao registrar aluno:", error);
        res.status(500).json({ erro: "Erro ao registrar aluno", detalhe: error.message });
    }
};
...
const loginAluno = async (req, res) => {
    ...
    try {
        const result = await pool.query("SELECT * FROM aluno WHERE email = $1", [email]);
        ...
        // Gerar token
        const token = jwt.sign({ id_aluno: aluno.id_aluno, email: aluno.email }, SECRET, { expiresIn: "1h" });

        res.json({ mensagem: "Login realizado com sucesso!", token });
    } catch (error) {
        console.error("Erro ao autenticar aluno:", error);
        res.status(500).json({ erro: "Erro ao autenticar aluno", detalhe: error.message });
    }
};
